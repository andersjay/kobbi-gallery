...
<div class="container px-8 pt-14 w-full mx-auto pb-10 border-t border-gray-800">
    <div class="max-w-7xl mx-auto">
        <h1 class="text-5xl text-gray-950 font-bold mb-10 text-center">{{ $artist->name }}</h1>
        @if($artist->artworks->count() > 0)
            ...
            <div class="flex flex-col items-center mb-12">
                <div class="w-full max-w-xl mx-auto mb-8">
                    @if(is_array($destaque->images) && count($destaque->images) > 0)
                        <img src="{{ asset('storage/' . $destaque->images[0]) }}" alt="{{ $destaque->name }}" class="w-full h-auto rounded-lg shadow-lg object-contain mx-auto cursor-pointer" onclick="openArtworkModal(0)">
                    @else
                    ...
                    <div class="mt-4 text-center">
                        ...
                        @if($destaque->description)
                            <p class="text-gray-600 text-base">{{ $destaque->description }}</p>
                        @endif
                    </div>
                </div>
                @if($outras->count() > 0)
                        ...
                        <div class="w-40 flex flex-col items-center">
                            <div class="aspect-square w-full rounded-lg overflow-hidden bg-gray-900 mb-2">
                                @if(is_array($artwork->images) && count($artwork->images) > 0)
                                    <img src="{{ asset('storage/' . $artwork->images[0]) }}" alt="{{ $artwork->name }}" class="w-full h-full object-cover cursor-pointer" onclick="openArtworkModal({{ $idx+1 }})">
                                @else
            ...
            <div id="artwork-modal" style="display:none; position:fixed; top:0; left:0; width:100vw; height:100vh; background:rgba(0,0,0,0.85); z-index:9999; align-items:center; justify-content:center;">
                ...
                <div class="flex items-center justify-center w-full h-full" id="modal-overlay-bg">
                    <div id="modal-content" class="relative flex flex-col md:flex-row items-center justify-center bg-white rounded-2xl shadow-2xl p-8 max-w-5xl w-full min-h-[500px] max-h-[80vh]">
                        <button id="modal-prev" class="hidden md:flex items-center justify-center bg-[#D1D1D1] hover:brightness-95 transition rounded-full w-12 h-12 absolute left-0 top-1/2 -translate-y-1/2 cursor-pointer shadow z-10 border-none" style="background:none;">
                            <svg width="32" height="32" viewBox="0 0 32 32"><polyline points="20,8 12,16 20,24" style="fill:none;stroke:black;stroke-width:3;stroke-linecap:round;stroke-linejoin:round"></polyline></svg>
                        </button>
                        <div id="modal-main-content" class="flex-1 flex items-center justify-center relative transition-all duration-300">
                            <img id="artwork-modal-img" src="" alt="" class="max-w-[32vw] max-h-[60vh] ml-8 rounded-lg shadow-lg bg-gray-200">
                        </div>
                        <div id="modal-info-content" class="flex-1 flex flex-col justify-center items-start md:pl-16 mt-8 md:mt-0 w-full max-w-md transition-all duration-300">
                            <div class="mb-6">
                            ...
                            <button id="modal-interest" class="bg-[#D1D1D1] text-black px-6 py-3 rounded font-medium text-base hover:brightness-95 transition border-none" style="border:none;">Registrar interesse</button>
                        </div>
                        <button id="modal-next" class="hidden md:flex items-center justify-center bg-[#D1D1D1] hover:brightness-95 transition rounded-full w-12 h-12 absolute right-0 top-1/2 -translate-y-1/2 cursor-pointer shadow z-10 border-none" style="background:none;">
                            <svg width="32" height="32" viewBox="0 0 32 32"><polyline points="12,8 20,16 12,24" style="fill:none;stroke:black;stroke-width:3;stroke-linecap:round;stroke-linejoin:round"></polyline></svg>
                        </button>
                    </div>
                    ...
                    <form id="interest-form" style="display:none;" class="absolute bg-white rounded-2xl shadow-2xl p-8 max-w-2xl w-full min-h-[500px] max-h-[80vh] flex flex-col justify-center">
                        ...
                        <div class="flex items-start mb-8">
                            <img id="form-artwork-img" src="" alt="" class="w-24 h-24 object-cover rounded mr-6 border border-gray-300">
                            <div>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" id="interest-name" name="name" required class="w-full border border-gray-400 rounded px-3 py-2 text-black">
                        </div>
                        <div class="mb-4">
                            <label for="interest-email" class="block font-semibold mb-1 text-black">E-mail *</label>
                            <input type="email" id="interest-email" name="email" required class="w-full border border-gray-400 rounded px-3 py-2 text-black">
                        </div>
                        <div class="mb-6">
                            <label for="interest-message" class="block font-semibold mb-1 text-black">Mensagem *</label>
                            <textarea id="interest-message" name="message" required class="w-full border border-gray-400 rounded px-3 py-2 min-h-[120px] text-black"></textarea>
                        </div>
                        <button type="submit" class="w-full bg-[#D1D1D1] text-black py-3 rounded font-semibold text-lg hover:brightness-95 transition">Enviar interesse</button>
                    </form>
                </div>
            </div>
            <script>
                ...
                function renderArtworkModal() {
                    ...
                    document.getElementById('modal-next').style.visibility = currentModalIdx < artworks.length - 1 ? 'visible' : 'hidden';
                }
                ...
                document.addEventListener('keydown', function(e) {
                    if(document.getElementById('artwork-modal').style.display === 'flex') {
                        ...
                        if(e.key === 'ArrowLeft' && currentModalIdx > 0) { currentModalIdx--; renderArtworkModal(); }
                        if(e.key === 'ArrowRight' && currentModalIdx < artworks.length - 1) { currentModalIdx++; renderArtworkModal(); }
                    }
                });
